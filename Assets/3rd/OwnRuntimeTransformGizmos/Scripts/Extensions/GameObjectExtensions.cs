...
namespace RTEditor
{
    ...
    public static class GameObjectExtensions
    {
        ...
        {
            GameObject clone = MonoBehaviour.Instantiate(gameObject) as GameObject;
            clone.name = gameObject.name;
            clone.layer = gameObject.layer;

            Transform cloneTransform = clone.transform;
            cloneTransform.localPosition = localPos;
            cloneTransform.localRotation = localRot;
        ...
        {
            var objectRoots = new List<GameObject>();
        ...
        {
            List<GameObject> allRoots = GetRootObjectsFromObjectCollection(gameObjects);
            List<GameObject> allRootsAndChildren = new List<GameObject>();
            foreach (GameObject rootObject in allRoots)
            {
                allRootsAndChildren.AddRange(rootObject.GetAllChildrenIncludingSelf());
            }

            return allRootsAndChildren;
        }
        ...
        public static List<GameObject> GetAllChildren(this GameObject gameObject)
        {
            ...
            {
                if (childTransform == objectTransform) continue;
        ...
        /// <summary>
        /// Goes up the hierarchy starting from 'gameObject' and returns the first
        /// empty parent object. It could be an immediate parent or a parent further
        /// up the hierarchy.
        /// </summary>
        public static GameObject GetFirstEmptyParent(this GameObject gameObject)
        {
            ...
            {
                if (parentTransform.gameObject.IsEmpty()) return parentTransform.gameObject;
        ...
        /// <summary>
        /// Goes up the hierarchy starting from 'gameObject' and returns the first
        /// parent object which has a component of type T attached to it. It could
        /// be an immediate parent or a parent further up the hierarchy.
        /// </summary>
        public static GameObject GetFirstParentOfType<T>(this GameObject gameObject) where T: Component
        {
            ...
            while (parentTransform != null)
            {
                ...
                if (component != null) return parentTransform.gameObject;
        ...
        public static void SetAbsoluteScale(this GameObject gameObject, Vector3 absoluteScale)
        {
            ...
            if (Mathf.Abs(absoluteScale.x) < minScale) absoluteScale.x = minScale * Mathf.Sign(absoluteScale.x);
            if (Mathf.Abs(absoluteScale.y) < minScale) absoluteScale.y = minScale * Mathf.Sign(absoluteScale.y);
            if (Mathf.Abs(absoluteScale.z) < minScale) absoluteScale.z = minScale * Mathf.Sign(absoluteScale.z);
        ...
        public static Box GetNonMeshModelSpaceBox(this GameObject gameObject)
        {
            ...
            {
                Box modelSpaceBox = gameObject.GetColliderModelSpaceBox();

                // Note: If the box is not valid, use the non-collider system values (below)
                if (modelSpaceBox.IsValid()) return modelSpaceBox;
            }
        ...
        {
            Box modelSpaceBox = Box.GetInvalid();
            BoxCollider boxCollider = gameObject.GetComponent<BoxCollider>();
            if (boxCollider != null && boxCollider.enabled) modelSpaceBox = new Box(boxCollider.center, boxCollider.size);

            SphereCollider sphereCollider = gameObject.GetComponent<SphereCollider>();
            if (sphereCollider != null && sphereCollider.enabled)
            {
                Vector3 sphereBoxSize = Vector3.one * sphereCollider.radius * 2.0f;
                Box sphereModelSpaceBox = new Box(sphereCollider.center, sphereBoxSize);
                if (modelSpaceBox.IsValid()) modelSpaceBox.Encapsulate(sphereModelSpaceBox);
                else modelSpaceBox = sphereModelSpaceBox;
            }
            ...
            if (capsuleCollider != null && capsuleCollider.enabled)
            {
                ...
                Vector3 capsuleBoxSize = Vector3.zero;
                if(capsuleCollider.direction == 0) capsuleBoxSize = new Vector3(capsuleCollider.height, diameter, diameter);
                else
                if (capsuleCollider.direction == 1) capsuleBoxSize = new Vector3(diameter, capsuleCollider.height, diameter);
                else capsuleBoxSize = new Vector3(diameter, diameter, capsuleCollider.height);

                Box capsuleModelSpaceBox = new Box(capsuleCollider.center, capsuleBoxSize);
                if (modelSpaceBox.IsValid()) modelSpaceBox.Encapsulate(capsuleModelSpaceBox);
                else modelSpaceBox = capsuleModelSpaceBox;
            }
