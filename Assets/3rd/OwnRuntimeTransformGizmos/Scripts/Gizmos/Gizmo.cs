...
namespace RTEditor
{
    ...
    public abstract class Gizmo : MonoBehaviour
    {
        ...
        [SerializeField]
        protected bool[] _axesVisibilityMask = new bool[3] { true, true, true };
        #endregion
        ...
        {
            _axesVisibilityMask[axisIndex] = isVisible;
        }

        public bool GetAxisVisibility(int axisIndex)
        {
            return _axesVisibilityMask[axisIndex];
        }
        ...
        {
            return IsReadyForObjectManipulation() && _isDragging;
        }
        ...
        /// <summary>
        /// Sets an axis mask for the specified game object. The array must contain 3 boolean values
        /// (one for each axis) which specify the masked state. A value of true indicates that the 
        /// axis is not masked. A value of false indicates that the object is masked.
        /// </summary>
        public void SetObjectAxisMask(GameObject gameObj, bool[] axisMask)
        {
            if (axisMask == null || axisMask.Length < 3) return;
            bool[] maskClone = axisMask.Clone() as bool[];

            if (_objAxisMask.ContainsKey(gameObj)) _objAxisMask[gameObj] = maskClone;
            else _objAxisMask.Add(gameObj, maskClone);
        }

        /// <summary>
        /// Sets the masked state of the specified object for the specified axis.
        /// </summary>
        public void SetObjectAxisMask(GameObject gameObj, int axisIndex, bool isMasked)
        {
            if (axisIndex < 0 || axisIndex >= 3) return;

            if (_objAxisMask.ContainsKey(gameObj)) _objAxisMask[gameObj][axisIndex] = isMasked;
            else
            {
                bool[] axisMask = new bool[] { true, true, true };
                if (isMasked) axisMask[axisIndex] = false;

                _objAxisMask.Add(gameObj, axisMask);
            }
        }
        #endregion
        ...
        protected float CalculateGizmoScale()
        {
            ...
            if (!_preserveGizmoScreenSize) return _gizmoBaseScale;
        ...
        protected int[] GetSortedGizmoAxesIndices()
        {
            ...
            float bestDotProduct = Vector3.Dot(gizmoLocalAxes[0], _cameraTransform.forward);        // Assume that the X axis must be drawn first
            ...
            for (int axisIndex = 1; axisIndex < 3; ++axisIndex)
            {
                ...
                float dotProduct = Vector3.Dot(gizmoLocalAxes[axisIndex], _cameraTransform.forward);
        ...
        protected void UpdateShaderStencilRefValuesForGizmoAxisLineDraw(int axisIndex, Vector3 startPoint, Vector3 endPoint, float gizmoScale)
        {
            ...
            Vector3 rayDirection = _camera.orthographic ? -_cameraTransform.forward : _cameraTransform.position - rayOrigin;
        ...
        protected float[] GetMultiAxisExtensionSigns(bool adjustForBetterVisibility)
        {
            ...
            if (adjustForBetterVisibility)
            {
                ...
                Vector3 cameraLookVector = _cameraTransform.forward;
        ...
        private void TakeObjectTransformSnapshots(out List<ObjectTransformSnapshot> objectTransformSnapshots)
        {
            ...
            bool filter = true;
            if (GetGizmoType() == GizmoType.VolumeScale) filter = false;
            List<GameObject> objectWhichCanBeTransformed = GetParentsFromControlledObjects(filter);
