...
namespace Loonim{
	...
	public static class HsyRgb{
		...
		public static void ToRgb(ref float h,ref float s,ref float y){
			...
			float k = 0; // Intermediate variable.

			if (h >= 0 && h < 60) {           // Sector 0: 0?- 60?				k = s * h / 60;
				b = y - R * s - G * k;
				r = b + s;
				g = b + k;
			} else if (h >= 60 && h < 120) {  // Sector 1: 60?- 120?				k = s * (h - 60) / 60;
				g = y + B * s + R * k;
				b = g - s;
				r = g - k;
			} else if (h >= 120 && h < 180) { // Sector 2: 120?- 180?				k = s * (h - 120) / 60;
				r = y - G * s - B * k;
				g = r + s;
				b = r + k;
			} else if (h >= 180 && h < 240) { // Sector 3: 180?- 240?				k = s * (h - 180) / 60;
				b = y + R * s + G * k;
				r = b - s;
				g = b - k;
			} else if (h >= 240 && h < 300) { // Sector 4: 240?- 300?				k = s * (h - 240) / 60;
				g = y - B * s - R * k;
				b = g + s;
				r = g + k;
			} else {                          // Sector 5: 300?- 360?				k = s * (h - 300) / 60;
				r = y + G * s + B * k;
				g = r - s;
				b = r - k;
			}
		...
		public static void ToHsy(ref float r,ref float g,ref float b){
			...
			if (r == g && g == b) {            // Limit case.
				s = 0; 
				h = 0; 
			} else if ((r >= g) && (g >= b)) { // Sector 0: 0?- 60?				s = r - b;
				h = 60 * (g - b) / s;
			} else if ((g > r) && (r >= b)) {  // Sector 1: 60?- 120?				s = g - b;
				h = 60 * (g - r) / s  + 60;
			} else if ((g >= b) && (b > r)) {  // Sector 2: 120?- 180?				s = g - r;
				h = 60 * (b - r) / s + 120;
			} else if ((b > g) && (g > r)) {   // Sector 3: 180?- 240?				s = b - r;
				h = 60 * (b - g) / s + 180;
			} else if ((b > r) && (r >= g)) {  // Sector 4: 240?- 300?				s = b - g;
				h = 60 * (r - g) / s + 240;
			} else {                           // Sector 5: 300?- 360?				s = r - g;
				h = 60 * (r - b) / s + 300;
			}
